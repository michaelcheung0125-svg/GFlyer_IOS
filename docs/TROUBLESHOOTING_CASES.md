...
### 後續（2026-09-20，0.6.4 起，待實機確認）

使用者再次回報：完整清除後 iPhone **取不到目前位置**，Google 地圖停在
上一個模擬位置，影響其他要定位的 App。

pymobiledevice3 [issue #572](https://github.com/doronz88/pymobiledevice3/issues/572)
有兩個相符的回報：

- 模擬位置離真實位置很遠時，清除後 iOS 會進入「找不到目前位置」狀態（灰色
  圓圈停在模擬位置），可持續幾分鐘。**先把模擬位置設到真實位置附近再清除**，
  恢復快得多。
- 設定模擬位置之後連線斷過、再用新連線送清除，清除**完全無效**，但新連線
  仍可設定位置。GFlyer 在 App 重開、VPN 斷線後重建 session 就是這種情況。

因此「完整清除」改為：等進行中的傳送落地 → 用同一條連線把位置設到**最後
一次已知的真實位置**（`DeviceLocationService` 從非模擬定位記下，開始模擬前
也會讀一次系統快取）→ 等 2 秒讓其他 App 收到 → 清除並拆 session → 驗證。
不知道真實位置時，至少設回目前的模擬座標，讓清除由同一條連線負責。

若這樣仍然不夠，開關飛行模式的指引照舊保留。

### 備註

一般的「停止」不做 session 拆除（保留通道，蜂窩網路下次開始比較快），
訊息也不宣稱已恢復真實定位。要還原給其他 App 用時才用「完整清除」。
「停止」也沒有加上「先移回真實位置」這一步：它主要用在兩次模擬之間。
