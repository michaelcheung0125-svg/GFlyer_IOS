...
| 地點搜尋 | 搜尋地址或直接輸入經緯度座標 |
| 設定頁 | 後端狀態、LocalDevVPN 狀態與開關、目標 IP、通道測試、速度預設 |
...
3. 選模式（傳送／單點／路線／螺旋）與速度，按開始。VPN 未開時 GFlyer 會自動切換到 LocalDevVPN 開啟，約 1 秒後自動回來繼續。
4. 可暫停、繼續、循環播放；切到背景仍繼續。
5. **用完先按 Stop**（會自動恢復真實 GPS）。想關 VPN 的話，在設定頁按「關閉 LocalDevVPN」，或在「完整清除」時開啟「清除後同時關閉 LocalDevVPN」。
